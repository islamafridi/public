jQuery(document).ready(function ($) {
    ...
    function add_new_repeater_item(container_id, item_name, item_html, repeater_limit = null) {
        if (!$(`#${container_id}`).hasClass('active')) {
            $(`#${container_id}`).addClass('active');
        }

        var total_container = $(`#${container_id} .at-coll-container`).length;
        var repeater_id = $(`#${container_id} .at-coll-container`).length;

        if(repeater_limit && total_container >= repeater_limit) {
            show_snackbar('You have reached the maximum limit.');
            return;
        }
        ...
        if(container_id == 'at-custom-archive-fp') {
            new_container.find("*[id]").each(function () {
                if ($(this).attr("id")) {
                    var old_id = $(this).attr("id");
                    var new_id = old_id.replace(/\[\]/g, `${repeater_id}`);
                    $(this).attr("id", new_id);
                }
            });

            new_container.find("*[data-target]").each(function () {
                if ($(this).attr("data-target")) {
                    var old_data_target = $(this).attr("data-target");
                    var new_data_target = old_data_target.replace(/\[\]/g, `${repeater_id}`);
                    $(this).attr("data-target", new_data_target);
                }
            });
        }
    ...
    function update_repeater_names(container_id) {
        if (container_id == 'at-term-field') {
            $(`#${container_id} .at-coll-container`).each(function (index) {
                $(this).find("input[type=text]").attr("name", `home_terms[${index}][term_title]`);
                $(this).find("input[type=number]").attr("name", `home_terms[${index}][term_limit]`);
                $(this).find("input[type=hidden]").attr("name", `home_terms[${index}][term_id]`);
                $(this).find("input[type=checkbox]").attr("name", `home_terms[${index}][term_btm_ad]`);
            });
        } else if (container_id == 'at-single-faq-field') {
            $(`#${container_id} .at-coll-container`).each(function (index) {
                $(this).find("input[type=text]").attr("name", `single_faqs[${index}][question]`);
                $(this).find("textarea").attr("name", `single_faqs[${index}][answer]`);
            });
        } else if (container_id == 'at-faq-field') {
            $(`#${container_id} .at-coll-container`).each(function (index) {
                $(this).find("input[type=text]").attr("name", `faqs[${index}][question]`);
                $(this).find("textarea").attr("name", `faqs[${index}][answer]`);
            });
        } else if (container_id == 'at-custom-archive-fp') {
            $(`#${container_id} .at-coll-container`).each(function (index) {
                $(this).find("input[name^='custom_cat_posts'][type=text]").each(function () {
                    var name = $(this).attr('name');
                    var newName = name.replace(/custom_cat_posts\[\d+\]/, `custom_cat_posts[${index}]`);
                    $(this).attr('name', newName);
                    var id = $(this).attr('id');
                    if (id) {
                        var newId = id.replace(/custom_cat_post_bg_\d+/, `custom_cat_post_bg_${index}`);
                        $(this).attr('id', newId);
                    }
                });
                $(this).find("input[name^='custom_cat_posts'][type=hidden]").each(function () {
                    var name = $(this).attr('name');
                    var newName = name.replace(/custom_cat_posts\[\d+\]/, `custom_cat_posts[${index}]`);
                    $(this).attr('name', newName);
                });
                $(this).find("input[name^='custom_cat_posts'][type=color]").each(function () {
                    var name = $(this).attr('name');
                    var newName = name.replace(/custom_cat_posts\[\d+\]/, `custom_cat_posts[${index}]`);
                    $(this).attr('name', newName);
                });
                $(this).find("input[data-target]").each(function () {
                    var data_target = $(this).attr('data-target');
                    var new_data_target = data_target.replace(/custom_cat_post_bg_\d+/, `custom_cat_post_bg_${index}`);
                    $(this).attr('data-target', new_data_target);
                });
                
                $(this).find("ul[data-post-search-id]").each(function () {
                    var dpsi = $(this).attr('data-post-search-id');
                    var new_dpsi = dpsi.replace(/\[\]/g, `${index}`);
                    $(this).attr('data-post-search-id', new_dpsi);
                });
            });
        }
    }

    function media_uploader(title, target_url) {
    ...
    function at_init_repeater_term_search(container_id, term_limit) {
        ...
        $(document).on('keyup', '#' + container_id + ' .coll-term-search', delay(function () {
            ...
            request.fail(function (jqXHR, textStatus) {
                show_snackbar('Search request failed ' + textStatus);
            });
        }, 500));
        ...
        $(document).on('click', '#' + container_id + ' .coll-term-results ul li', function () {
            ...
            var new_term = $('<li data-term-id="' + term_id + '">' + term_title + '<span class="delete"><i class="fa fa-trash-alt"></i></span><input type="hidden" name="home_terms[' + data_term_search_id + '][term_id]" value="' + term_id + '" /></li>').hide();
    ...
    function at_init_repeater_post_search(container_id, post_limit) {
        $(document).on('click', '#' + container_id + ' .coll-post-search', function (e) {
            e.stopPropagation();
        });

        $('body').on('click', function (e) {
            if (!$(e.target).hasClass('coll-post-search') && !$(e.target).hasClass('coll-post-results')) {
                $('.' + container_id + ' .coll-post-results').hide();
            }
        });

        $(document).on('click', '#' + container_id + ' .coll-post-search', function () {
            var search = $(this).val();
            if (search.length >= 3) {
                $('.' + container_id + ' .coll-post-results').show();
            }
        });

        $(document).on('keyup', '#' + container_id + ' .coll-post-search', delay(function () {
            var search_input = $(this);
            var search = search_input.val();

            if (search.length == 0) {
                search_input.siblings('.coll-post-results').hide();
                return;
            }
        ...
        $(document).on('click', '#' + container_id + ' .coll-post-selected ul li .delete', function () {
            ...
            selected_container.append('<input type="hidden" name="custom_cat_posts[' + data_post_search_id + '][post]" value="" />');
        });
        ...
        $(document).on('click', '#' + container_id + ' .coll-post-results ul li', function () {
            ...
            var new_post = $('<li data-post-id="' + post_id + '">' + post_title + '<span class="delete"><i class="fa fa-trash-alt"></i></span><input type="hidden" name="custom_cat_posts[' + data_post_search_id + '][post]" value="' + post_id + '" /></li>').hide();

            selected_container.find("input[name='custom_cat_posts[" + data_post_search_id + "][post]']").remove();

            if (exist_posts.length >= post_limit) {
                show_snackbar('You have reached the maximum limit of ' + post_limit + ' post.');
                return;
            }

            var exist_posts_ids = $(this).closest('.at-coll-container').find('.coll-post-selected ul li').map(function () {
            ...
            $(this).closest('.at-coll-container').find('.coll-post-selected ul').append(new_post);
            new_post.slideDown(250);

            $(this).closest('.at-coll-container').find('.coll-post-search').val('');
        });
    }

    function at_init_term_search(container_id, save_key, term_limit) {
        $(document).on('click', '#' + container_id + ' .at-search-ipt', function (e) {
            e.stopPropagation();
        });

        $('body').on('click', function (e) {
            if (!$(e.target).hasClass('at-search-ipt') && !$(e.target).hasClass('term-results')) {
                $('#' + container_id + ' .term-results').hide();
            }
        });
        ...
        $(document).on('click', '#' + container_id + ' .at-search-ipt', function () {
            var search = $(this).val();
            if (search.length >= 3) {
                $('#' + container_id + ' .term-results').show();
            }
        });
        ...
        $(document).on('keyup', '#' + container_id + ' .at-search-ipt', delay(function () {
            ...
            if (search.length == 0) {
                search_input.siblings('.term-results').hide();
                return;
            }
            ...
            var request = $.ajax({
                ...
                success: function (data) {
                    search_input.siblings('.term-results').show().html(data);
                }
            });
        ...
        $(document).on('click', '#' + container_id + ' .term-selected ul li .delete', function () {
            $(this).parent().fadeOut(350, function () {
                $(this).remove();
            });
        ...
        $(document).on('click', '#' + container_id + ' .term-results ul li', function () {
            var term_id = $(this).data('term-id');
            var term_title = $(this).text();
            var exist_terms = $('#' + container_id + ' .term-selected ul li');
            var new_term = $('<li data-term-id="' + term_id + '">' + term_title + '<span class="delete"><i class="fa fa-trash-alt"></i></span><input type="hidden" name="' + save_key + '" value="' + term_id + '" /></li>').hide();

            $('#' + container_id + ' .term-selected ul').find("input[name='" + save_key + "']").remove();
            ...
            var exist_terms_ids = $('#' + container_id + ' .term-selected ul li').map(function () {
                return $(this).data('term-id');
            }).get();
            ...
            $('#' + container_id + ' .term-selected ul').append(new_term);
            new_term.slideDown(250);
    ...
    $(document).on('click', '#apkt-license-btn', function (e) {
        ...
        request.fail(function (jqXHR, textStatus, errorThrown) {
            panel_form.removeClass('loading');
            show_snackbar("License activation error: " + textStatus);
        });
    });
    ...
    $(document).on('change', '#import-demo', function (e) {
        ...
        if (import_file) {
            if (import_file.name.toLowerCase().includes('demo') && import_file.name.toLowerCase().endsWith('.json')) {
                panel_form.addClass('loading');
			   ...
			   show_snackbar("Importing...", false);
                ...
                snackbar.css('opacity', '0');
                show_snackbar('Please select a correct demo file or .json file to import.');
            }
        } else {
            snackbar.css('opacity', '0');
        ...
        if (import_file) {
            if (import_file.name.toLowerCase().endsWith('.json')) {
                ...
                request.fail(function (jqXHR, textStatus, errorThrown) {
                    panel_form.removeClass('loading');
                    snackbar.css('opacity', '0');
                    show_snackbar("The following error occurred: " + textStatus);
                });
            } else {
                snackbar.css('opacity', '0');
    ...
    $(document).on('click', '#export-settings', function (e) {
        ...
        if (confirm_export) {
            ...
            request.fail(function (jqXHR, textStatus, errorThrown) {
                panel_form.removeClass('loading');
                show_snackbar("TExport settings failed: " + textStatus);
            });
        } else {
            return;
        }
    ...
    $(document).on('click', '#delete-cache', function (e) {
        ...
        request.fail(function (jqXHR, textStatus, errorThrown) {
            panel_form.removeClass('loading');
            show_snackbar("Clear cache failed: " + textStatus);
        });
    });

    $(document).on('submit', '#at-panel-form', function (e) {
        e.preventDefault();
        if ($('#at-term-field .at-coll-container').length === 0) {
            $('<input>').attr({
                type: 'hidden',
                name: 'home_terms[]',
                value: ''
            }).appendTo($('#at-term-field'));
        }

        if ($('#at-single-faq-field .at-coll-container').length === 0) {
            $('<input>').attr({
                type: 'hidden',
                name: 'single_faqs[]',
                value: ''
            }).appendTo($('#at-single-faq-field'));
        }

        if ($('#at-faq-field .at-coll-container').length === 0) {
            $('<input>').attr({
                type: 'hidden',
                name: 'faqs[]',
                value: ''
            }).appendTo($('#at-faq-field'));
        }

        if ($('#at-custom-archive-fp .at-coll-container').length === 0) {
            $('<input>').attr({
                type: 'hidden',
                name: 'custom_cat_posts[]',
                value: ''
            }).appendTo($('#at-custom-archive-fp'));
        }
    ...
    const term_html = '<div class="at-coll-container">' +
        '    <div class="at-coll-header">' +
        '        <div class="at-coll-title">Term 1</div>' +
    ...
    const single_faq_html = '<div class="at-coll-container">' +
        '    <div class="at-coll-header">' +
        '        <div class="at-coll-title">FAQ 1</div>' +
        '        <div class="at-coll-action">' +
        '            <span class="at-action-move">' +
        '            <i class="fa fa-bars"></i>' +
        '            </span>' +
        ...
        '            <input type="text" name="single_faqs[][question]" class="at-text-ipt" />' +
        '        </div>' +
        '        <div>' +
        '            <p class="at-mini-title">Answer</p>' +
        '            <p>Write answer here.</p>' +
        '            <textarea name="single_faqs[][answer]" class="at-textarea" rows="5"></textarea>' +
        '        </div>' +
        '    </div>' +
        '</div>';

    const faq_html = '<div class="at-coll-container">' +
        '    <div class="at-coll-header">' +
        '        <div class="at-coll-title">FAQ 1</div>' +
        '        <div class="at-coll-action">' +
        '            <span class="at-action-move">' +
        ...
        '            <p class="at-mini-title">Question</p>' +
        '            <p>Write question here.</p>' +
        '            <input type="text" name="faqs[][question]" class="at-text-ipt" />' +
        '        </div>' +
        '        <div>' +
        '            <p class="at-mini-title">Answer</p>' +
        '            <p>Write answer here.</p>' +
        '            <textarea name="faqs[][answer]" class="at-textarea" rows="5"></textarea>' +
        '        </div>' +
        '    </div>' +
        '</div>';
    
        const cap_html = '<div class="at-coll-container">' +
        '   <div class="at-coll-header">' +
        '       <div class="at-coll-title">post 1</div>' +
        '       <div class="at-coll-action">' +
        ...
        '               <input type="text" name="custom_cat_posts[][bg_image]" id="custom_cat_post_bg_[]" class="at-text-ipt" value="" />' +
        '               <input type="button" class="at-upload-img-ipt" value="&#xf093;" data-title="Custom Archive Page Posts Background Image" data-target="#custom_cat_post_bg_[]" />' +
        '           </div>' +
        '       </div>' +
        '       <div class="at-mb-2 at-pb-2">' +
        '           <p class="at-mini-title">Background Color</p>' +
        '           <p>Set background color which suitable for background image.</p>' +
        '           <input type="text" name="custom_cat_posts[][bg_color]" value="#FFC541" class="color-picker" />' +
        '       </div>' +
        '       <div>' +
        ...
        '                           <input type="hidden" name="custom_cat_posts[][post]" value="" />' +
        '                       </ul>' +
        '                   </div>' +
        '               </div>' +
        '           </div>' +
    ...
    at_init_repeater('at-term-field', 'Term');
    at_init_repeater('at-single-faq-field', 'FAQ');
    at_init_repeater('at-faq-field', 'FAQ');
    at_init_repeater('at-custom-archive-fp', 'Post');

    at_init_repeater_term_search('at-term-field', 1);
    at_init_repeater_post_search('at-custom-archive-fp', 1);

    at_init_term_search('at-custom-cat-search', 'custom_cat', 1);

    $(document).on('click', '#add-new-term', function () {
        add_new_repeater_item('at-term-field', 'Term', term_html);
    });

    $(document).on('click', '#add-new-sp-faq', function () {
        add_new_repeater_item('at-single-faq-field', 'FAQ', single_faq_html);
    });

    $(document).on('click', '#add-new-faq', function () {
        add_new_repeater_item('at-faq-field', 'FAQ', faq_html);
    });

    $(document).on('click', '#add-new-custom-archive-fp', function () {
        add_new_repeater_item('at-custom-archive-fp', 'Post', cap_html, 4);
