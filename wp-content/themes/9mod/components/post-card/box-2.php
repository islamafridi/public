...
<h3>
  ...
  $some_days_ago = strtotime('-360 days');
  $threshold = 86400; // 1 day in seconds

  if ($updated_timestamp >= $published_timestamp + $threshold) {
      if ($updated_timestamp > $some_days_ago) :
  ?>
      <span class="updated"><?php theme_translate('Updated', 'Updated_postcard'); ?> </span>
  <?php
      endif;
  } else {
      if ($published_timestamp > $some_days_ago) :
  ?>
      <span class="new"><?php theme_translate('New', 'New_postcard'); ?></span>
  <?php
      endif;
  }
  ?>
